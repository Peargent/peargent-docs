...
Import the text extraction tool from Peargent:
...
from peargent.tools import text_extractor // [!code highlight:3]
...
Combine the text extraction tool with other tools for complex workflows. Each agent can be configured with different **<u>[Models](/docs/models)</u>** based on your requirements:
...
agent = create_agent(
    ...
    description="Processes documents and performs various operations",
    persona="You are a document processing expert",
    model=anthropic("claude-3-5-sonnet-20241022"),
    tools=[text_extractor]  # Add other tools as needed // [!code highlight]
)

# Agent can now extract text and perform operations on it
response = agent.run("Extract text from this PDF and identify all email addresses")
```
...
### Example 6: Conditional Text Extraction

```python
from peargent.tools import text_extractor
import os

def safe_extract_text(file_path: str, max_length: int = None) -> str:
...
# Usage
text = safe_extract_text("document.docx", max_length=2000)
print(text)
```

## Security Features

The Text Extraction Tool includes built-in security features:

- **SSRF Protection**: URL validation prevents access to localhost and private IP addresses
- **Scheme Validation**: Only HTTP and HTTPS URLs are allowed
- **Encoding Safety**: Automatic fallback encoding detection for text files
- **Error Handling**: Graceful error handling with detailed error messages

## Dependencies

Some document formats require optional dependencies:

- **HTML extraction**: `beautifulsoup4` (`pip install beautifulsoup4`)
- **PDF extraction**: `pypdf>=6.0.0` (`pip install pypdf`)
- **DOCX extraction**: `python-docx` (`pip install python-docx`)
